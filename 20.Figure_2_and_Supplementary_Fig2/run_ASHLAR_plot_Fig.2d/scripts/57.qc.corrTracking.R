...
trackDir <- file.path(params$analysisDir, params$ashlarAnalysisDir, "45.track_cellpool_to_ashlar")
...
trackDat <- map(wells, function(w) {
        file.path(trackDir, paste0(w, ".", cyc1, "_", cyc2, 
                        "_tracking", params$trackingParams$stg2OutSuffix)) %>%
...
map(wells, function(w) {
	...
	write.table(this, 
			file = paste0(w, ".", cyc1, "_", cyc2,
                                                "_potential_errors",
                                                params$trackingParams$stg2OutSuffix),
...
ggsave(paste0(cyc1, "_", cyc2, "_stg2.consistency_of_mate_displacement_wrt_local_average.",
