...
well = sys.argv[1]
wellOmeTiff = '/Users/krishnachoudhary/Documents/Analysis/10.U2OS-tinypool/70.run_ASHLAR/10.take1/20.mosaics/' + well + '_maxShift_15_sigma_3.ome.tiff'
paramsFile = '/Users/krishnachoudhary/Documents/Analysis/10.U2OS-tinypool/70.run_ASHLAR/10.take1/00.config/config.json'
...
nucChs = [0, 2, 6, 10]
with tifffile.TiffFile(wellOmeTiff) as tif:
	for nxCh in nucChs:
		page = tif.series[0][nxCh]
		im = page.asarray()*65535
		dps['ch' + str(nxCh)] = expandBboxAndExtract(mask, im, dpWidth)
		dps['ch' + str(nxCh)]['images'] = OrderedDict()
		for lbl in range(nSamples):
			dps['ch' + str(nxCh)]['images']['n' + \
				str(dps['ch' + str(nxCh)]['label'].iloc[lbl])] = \
					dps['ch' + str(nxCh)]['cellSnaps'][lbl]
		dps['ch' + str(nxCh)] = dps['ch' + str(nxCh)]['images'] 
		del page	

indMasks = expandBboxAndExtract(mask, mask, dpWidth)

normalizers = dict(zip(['ch' + str(x) for x in nucChs], \
	[normalizers[x] for x in ['C2-cyc1', 'C4-cyc2', 'C4-cyc3', 'C4-cyc4']]))
...
outmage2.save(os.path.join('/tmp/lima', f"{wellOrder:02d}" + '.png'))
