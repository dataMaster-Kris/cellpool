...
smpIds <- data.frame(
  filename = c("export_no-virus-anti-CD81-stained_Single Cells - SSC.csv", 
               "export_pV2TUd12_CD81ko_and_Cre_anti-CD81-stained_Single Cells - SSC.csv", 
               "export_pV2TUdCtrl2_NTC_anti-CD81-stained_Single Cells - SSC.csv"),
...
nClass <- data.frame(id = c("no-virus-anti-CD81-stained",
                            "pV2TUdCtrl2_NTC",
                            "pV2TUd12_CD81ko"),
                     n = c(100, 100, 30))
GFP_ePDF <- list()
for (id in unique(dat$id)) {
  thisHist <- hist(dat$gfp[(dat$id == id) & 
                             ((dat$gfp > 0.1) | 
                                (dat$id != "pV2TUd12_CD81ko"))],
                   plot = FALSE,
                   nclass = nClass$n[nClass$id == id])
...
ggsave(paste0("p1_wtCas9_CD81ko_comparisons_", Sys.time(),".pdf"),
       plot = p1NoText,
       width = 0.6*2.5, height = 1.7*2.5/3.8)
ggsave(paste0("p2_wtCas9_CD81kocomparisons_", Sys.time(),".pdf"),
       plot = p2NoText,
       width = 0.6*2.5, height = 1.7*2.5/3.8)
